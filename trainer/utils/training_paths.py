...
def get_image_base_model_path(model_id: str) -> str:
    ...
    if os.path.isdir(base_path):
        # 1. Check if it's a full Diffusers repo (has model_index.json)
        if os.path.exists(os.path.join(base_path, "model_index.json")):
            return base_path
            
        # 2. If not, look for the first .safetensors file regardless of other files
        for f in os.listdir(base_path):
            if f.endswith(".safetensors"):
                return os.path.join(base_path, f)
                
        # 3. Fallback to directory path (or model_id) if nothing found
        return base_path
