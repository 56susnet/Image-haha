...
def create_config(task_id, model_path, model_name, model_type, expected_repo_name, trigger_word: str | None = None, optimization_overrides: dict | None = None):
    ...
    if is_ai_toolkit:
        ...
        # --- Dynamic Epochs (User Requested: FINAL OVERRIDE) ---
        if model_type == "sdxl":
            ...
            if est_batch_size <= 0: est_batch_size = 4
            ...
            if 1 <= num_images <= 20:
                ...
                total_epochs = int(config.get("max_train_epochs", 30)) # Fallback if not mapped

            # Apply Dynamic Epochs (Forcefully)
            config["max_train_epochs"] = total_epochs
            # Recalculate save frequency based on new epoch count
            config["save_every_n_epochs"] = max(1, math.ceil(total_epochs / 4))
            print(f"  [Dynamic:FINAL] Epochs set to {total_epochs} (Images: {num_images}, Est. Hours: {hours_to_complete:.2f})", flush=True)
